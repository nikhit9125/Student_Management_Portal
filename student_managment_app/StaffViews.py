...
from .models import CustomeUser, Staffs, Courses, Subjects, Students, SessionYearModel, Attendence, AttendenceReport, LeaveReportStaff, FeedBackStaffs, StudentResult
...
def staff_home(request):
	...
	for student in students_attendance:
		attendance_present_count = AttendenceReport.objects.filter(status=True,
																student_id=student.id).count()
		attendance_absent_count = AttendenceReport.objects.filter(status=False,
																student_id=student.id).count()
...
def save_attendance_data(request):
	...
	try:
		...
		attendance = Attendence(subject_id=subject_model,
		...
		for stud in json_student:
			...
			attendance_report = AttendenceReport(student_id=student,
...
def update_attendance_data(request):
	...
	try:
		...
		for stud in json_student:
			...
			attendance_report = AttendenceReport.objects.get(student_id=student,
															attendance_id=attendance)
...
def staff_profile_update(request):
	if request.method != "POST":
		...
		try:
			...
			staff = Staffs.objects.get(admin=customeuser.id)
